...
%Lecture des defaut jaune 
defaut = zeros(1,1,3);
defaut(1,1,:) = [255 255 0];
defaut = repmat(defaut, nb_lignes,nb_colonnes);
D = (u0 == defaut);
...
D = reshape(D,[nb_pixels 3]);
W_d = spdiags(1-D(:),0,nb_pixels,nb_pixels);
size(W_d)
...
% Point fixe :
lambda = 1;			% Poids de la rgularisation
u_k = u0;
convergence = +Inf;
iteration = 0;
