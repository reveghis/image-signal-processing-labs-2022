...
ub = double(imread('Images/defaut_fleur.png'));
[ub_nb_lignes,ub_nb_colonnes,ub_nb_canaux] = size(ub);
ub = ub>0;
...
ub = reshape(ub,[nb_pixels ub_nb_canaux]);
W_d = spdiags(1-ub,0,nb_pixels,nb_pixels);
size(W_d)
...
% Point fixe :
lambda = 5;			% Poids de la rgularisation
u_k = u0;
convergence = +Inf;
iteration = 0;
...
while convergence > 1e-3
	...
	% Itration (6) :
    for i=1:nb_canaux
	    u_kp1(:,i) = inpainting(b(:,i),u_k(:,i),lambda,Dx,Dy,epsilon,W_d);
    end
