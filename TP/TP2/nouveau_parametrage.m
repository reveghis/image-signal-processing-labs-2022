...
% Affichage des points de contrôle (disques noirs) :
d = length(beta)+1;
alpha_0 = 0;
alpha_d = 1;
alpha = [1:d-1]/d;
...
% Tracé de la droite reliant les points de contrôle extrêmes (trait bleu) :
line([alpha_0 alpha_d],[beta_0 beta_d],'Color','b','LineWidth',3);

% Affichage des projections verticales des points de contrôle sur cette droite (disques bleus) :
beta_barre = beta_0+(beta_d-beta_0)*alpha;
plot(alpha,beta_barre,'ob','MarkerFaceColor','b','MarkerSize',10,'LineWidth',3);

% Matérialisation des écarts (pointillés noirs) :
for i = 1:d-1
